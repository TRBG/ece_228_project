...
% This script will read the images from the Fish4Knowledge dataset and
...
% fish_23 for the fish images and mask_01 to mask_23 for the mask images)
...
% data_path = 'D:\Phd\UCSD Related\Courses\ECE Courses\ECE 228\Project\code\data';
% augmanted_data_path = 'D:\Phd\UCSD Related\Courses\ECE Courses\ECE 228\Project\code\augmented_data';
...
for i = 1 : length(subFolders)/2
    ...
    % Checking the number of examples in this class as a ratio to 10,000
    % Which is almost equal to the number of images in the class with the
    % highest number of examples
    factor_to_balanced = 10000/length(images_files_list);
    
    % If the ratio is too low (we are comparing the reciprocal in the if
    % statement below) we augment
    
    if factor_to_balanced > 10
        ...
        for j = 1 : length(images_files_list)
            ...
            for k = 1: floor(factor_to_balanced/5)
